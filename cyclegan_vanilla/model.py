...
class cyclegan(object):
    ...
    def test(self, args):
        ...
        if args.which_direction == 'AtoB':
            ...
            # sample_files = glob('./datasets/{}/train/*.*'.format(self.dataset_A_dir))
        elif args.which_direction == 'BtoA':
            sample_files = glob('./datasets/{}/test/*.*'.format(self.dataset_B_dir))
            # sample_files = glob('./datasets/{}/train/*.*'.format(self.dataset_B_dir))
        else:
        ...
        for idx in range(len(sample_files)):
            ...
            source_matrix, transfer_matrix = np.squeeze(origin_midi*1.0), np.squeeze(fake_midi*1.0)
            ...
            content_retention.append(retention)
        ...
        print("mean of content retention maxtrix: (added, removed, total) =", retention_mean)
        print("variance of content retention maxtrix: (added, removed, total) =", retention_variance)
