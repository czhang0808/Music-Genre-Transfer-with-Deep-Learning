...
class Classifer_new(object):
    def __init__(self, sess, args):
        ...
        self.discriminator = discriminator_classifier_new
    ...
    def _build_model(self):
        ...
        self.origin_train = tf.placeholder(tf.float32, [self.batch_size, self.time_step, self.pitch_range,
                                                        self.input_c_dim])
        self.label_train = tf.placeholder(tf.float32, [self.batch_size, 3])
        self.origin_test = tf.placeholder(tf.float32, [None, self.time_step, self.pitch_range, self.input_c_dim])
        self.label_test = tf.placeholder(tf.float32, [None, 3])
    ...
    def train(self, args):
        ...
        # log path
        log_dir = './logs/classifier_new_{}_{}'.format(self.now_datetime, str(self.sigma_c))
        self.writer = tf.summary.FileWriter(log_dir, self.sess.graph)
        counter = 1

        # create training list (origin data with corresponding label)
        # Label for A is (1, 0), for B is (0, 1)
        dataA = glob('./datasets/JC_C/train/*.*')
        dataB = glob('./datasets/JP_P/train/*.*')
        dataC = glob('./datasets/JC_J/train/*.*')
        labelA = [(1.0, 0.0, 0.0) for _ in range(len(dataA))]
        labelB = [(0.0, 1.0, 0.0) for _ in range(len(dataB))]
        labelC = [(0.0, 0.0, 1.0) for _ in range(len(dataC))]
        data_origin = dataA + dataB + dataC
        label_origin = labelA + labelB + labelC
        training_list = []
        ...
        dataA = glob('./datasets/JC_C/test/*.*')
        dataB = glob('./datasets/JP_P/test/*.*')
        dataC = glob('./datasets/JC_J/test/*.*')
        labelA = [(1.0, 0.0, 0.0) for _ in range(len(dataA))]
        labelB = [(0.0, 1.0, 0.0) for _ in range(len(dataB))]
        labelC = [(0.0, 0.0, 1.0) for _ in range(len(dataC))]
        data_origin = dataA + dataB +dataC
        ...
        label_test = np.array(label_test).astype(np.float32).reshape(len(label_test), 3)
        ...
        for epoch in range(args.epoch):
            ...
            for idx in range(batch_idx):
                ...
                batch_label = np.array(batch_label).astype(np.float32).reshape(len(batch_label), 3)
    ...
    def save(self, checkpoint_dir, step):
        ...
        model_dir = "classifier_new_{}_{}".format(self.now_datetime, str(self.sigma_c))
    ...
    def test(self, args):
        ...
        count_transfer = 0
        count_cycle = 0
        count_out_class_origin = 0
        count_out_class_transfer = 0
        line_list = []
        ...
        for idx in range(len(sample_files)):
            ...
            # count # of out of class results, i.e. classified as genre C 
            count_out_class_origin += 1 if np.argmax(test_result_origin[0]) == 2 else 0
            count_out_class_transfer += 1 if np.argmax(test_result_transfer[0]) == 2 else 0
        ...
        accuracy_out_origin = count_out_class_origin * 1.0 / len(sample_files)
        accuracy_out_transfer = count_out_class_transfer * 1.0 / len(sample_files)
        print('Accuracy of this classifier on test datasets is :', accuracy_origin, accuracy_transfer, accuracy_cycle)
        print('Accuracy of out of class results :', accuracy_out_origin, accuracy_out_transfer)
