...
def conv2d(input_, output_dim, ks=7, s=2, stddev=0.02, padding='SAME', name="conv2d", sn = True):
    with tf.variable_scope(name):
        if sn:
...
def deconv2d(input_, output_dim, ks=7, s=2, stddev=0.02, padding='SAME', name="deconv2d", sn = True):
    with tf.variable_scope(name):
